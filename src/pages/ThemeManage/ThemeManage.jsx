    ...
    handleOk = () => {
        ...
        this.props.form.validateFields((err, values) => {
            ...
            this.props.dispatch({
                ...
                payload: {
                    ...values,
                    id: this.state.saveModal.current ? this.state.saveModal.current.id : 'create',
    ...
    render() {
        const column = [
            ...
            },
            {
                ...
                dataIndex: 'banner',
                render: value => (
                    <img src={value} style={{width: '30px'}} alt=""/>
                )
            },
            {
                title: '学习概要',
            ...
            },
            {
                ...
                render: (_, item, index) => (
                    <div>
                        <span className='link' onClick={() => this.editTheme(item)}>编辑</span>
                        <Popconfirm title='确认删除吗？' okText='确认' cancelText='取消' onConfirm={() => this.delete(item)}>
                            <span className='link'>删除</span>
                        </Popconfirm>
                        <span className='link' onClick={() => this.addStepModal(item)}>步骤添加</span>
                    </div>
                )
            },
        ]
        const { fileList, fileList1, fileList2, previewVisible, previewImage, previewVisible1, previewImage1, previewVisible2, previewImage2 } = this.state;
        ...
        return (
            <PageHeaderWrapper extra={<Button type='primary' onClick={this.addTheme}>主题添加</Button>}>
                <Table 
                    columns={column}
                    ...
                    <Form.Item {...formItemLayout} label='所属阶段'>
                        {getFieldDecorator('sid', 
                        {
                            ...
                            <Select placeholder='所属阶段'>
                                {this.props.theme.stageList.map(item => <Select.Option value={item.id}>{item.title}</Select.Option>)}
